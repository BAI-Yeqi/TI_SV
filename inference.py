...
def main():
    ...
    parser.add_argument("--mode", default="infer",
                        choices=["train", "test", "infer"], help="setting mode for execution")
    ...
    # LSTM
    parser.add_argument("--num_lstm_stacks", type=int,
                        default=3, help="number of LSTM stacks")
    parser.add_argument("--num_lstm_cells", type=int,
                        default=768, help="number of LSTM cells")
    parser.add_argument("--dim_lstm_projection", type=int,
                        default=256, help="dimension of LSTM projection")
...
# Collect hparams
    ...
    # feeder = Feeder(args)
    feeder = Feeder(args.in_wav1, args.in_wav2)
    feeder.preprocess()
    ...
    with tf.Session(graph=graph) as sess:
        ...
        wav1_out = sess.run(model.norm_out, feed_dict={
                            model.input_batch: wav1_data})
        wav2_out = sess.run(model.norm_out, feed_dict={
                            model.input_batch: wav2_data})

        wav1_dvector = np.mean(wav1_out, axis=0)
        wav2_dvector = np.mean(wav2_out, axis=0)

        final_score = dot(wav1_dvector, wav2_dvector) / \
            (norm(wav1_dvector)*norm(wav2_dvector))
